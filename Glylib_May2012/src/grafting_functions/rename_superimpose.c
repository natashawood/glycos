...
/****************************************************SCENARIO 4****************************************************************/
if (strcmp(core_aN,"O8")==0){
	scenario=4; //2-8 linkages!
	printf("SCENARIO 4: 2-8 linkages\n\n");
	nb=0; // so we don't do anything else later
	for (ai=0;ai<(*A).m[c_mi][0].r[c_ri].na;ai++){
		if (strcmp((*A).m[c_mi][0].r[c_ri].a[ai].N,"C6")==0){strcpy((*A).m[c_mi][0].r[c_ri].a[ai].N,"a1");}
		if (strcmp((*A).m[c_mi][0].r[c_ri].a[ai].N,"C7")==0){strcpy((*A).m[c_mi][0].r[c_ri].a[ai].N,"a2"); A_a2=&(*A).m[c_mi][0].r[c_ri].a[ai];}
		if (strcmp((*A).m[c_mi][0].r[c_ri].a[ai].N,"C5")==0){strcpy((*A).m[c_mi][0].r[c_ri].a[ai].N,"a3");}
		if (strcmp((*A).m[c_mi][0].r[c_ri].a[ai].N,"O6")==0){strcpy((*A).m[c_mi][0].r[c_ri].a[ai].N,"a4");}		
	}
	strcpy(cplx_a1, "C6");
	strcpy(cplx_a2, "C7");
	strcpy(cplx_a3, "C5");
...
for(mi=0;mi<(*B).nm;mi++){ // for each molecule
		...
		if((*B).m[mi][0].r[ri].n==Tcore_rn){ // if residue number is the templates core residue number
				...
				// Any atoms from 7 to 9 should be replaced with the equivalent atom in A.
				if(scenario==4){ // 2-8 linkages. Swap out certain linkage atoms for the grafted branch ones
					strcpy(Bname,(*B).m[mi][0].r[ri].a[ai].N);
					if ( (Bname[1]=='7') || (Bname[1]=='8') || (Bname[1]=='9') ){
						for (i=0;i<(*A).m[c_mi][0].r[c_ri].na;i++){
							if (strcmp((*B).m[mi][0].r[ri].a[ai].N,(*A).m[c_mi][0].r[c_ri].a[i].N)==0){
								(*B).m[mi][0].r[ri].a[ai].x.i=(*A).m[c_mi][0].r[c_ri].a[i].x.i;
								(*B).m[mi][0].r[ri].a[ai].x.j=(*A).m[c_mi][0].r[c_ri].a[i].x.j;
								(*B).m[mi][0].r[ri].a[ai].x.k=(*A).m[c_mi][0].r[c_ri].a[i].x.k;
							}
						}
					}
				}	
...
//a2 atom: Removes A's(branches). B's already has A's coords. B should get atom name and number from A too. 
//Otherwize it would remain ROH O1. Now that there is a different link it needs to change. Only happens in scenario 1.
if (scenario==1){
	...
	strcpy((*B).m[ROHmi][0].r[ROHri].a[ROHai].N,ROHatom);
        (*B).m[ROHmi][0].r[ROHri].a[ROHai].n=(*A).m[b_mi][0].r[b_ri].a[b_ai].n; //Change atom number too.
