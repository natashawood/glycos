...
int find_vdw_clashes_return_self_clashes(assembly *A){ 
int mi=0,mii=0,ri=0,rii=0,ai=0,aii=0;//loop counters 
...
printf("Entered self Clash function\n");

// Wiggling produces self clashes sometimes. This will check if any new "bonds" are formed
// If bonds are new:

//for(mi=nbr+1;mi<(*A).nm;mi++){
for(mi=0;mi<(*A).nm;mi++){
        for(ri=0;ri<(*A).m[mi][0].nr;ri++){
                for(ai=0;ai<(*A).m[mi][0].r[ri].na;ai++){// ai is now current atom to check against every atom in assembly 
			for(mii=0;mii<(*A).nm;mii++){
				for(rii=0;rii<(*A).m[mii][0].nr;rii++){
					if( (mi!=mii) || (ri!=rii) ) { // Dont check the same residue against itself
						//printf("Checking %d against %d\n",ri,rii);
						for(aii=0;aii<(*A).m[mii][0].r[rii].na;aii++){
							//printf("Comparing Values\n rA=%f\n",rA);
                                			x=((*A).m[mi][0].r[ri].a[ai].x.i-(*A).m[mii][0].r[rii].a[aii].x.i);
                                			y=((*A).m[mi][0].r[ri].a[ai].x.j-(*A).m[mii][0].r[rii].a[aii].x.j);
                                			z=((*A).m[mi][0].r[ri].a[ai].x.k-(*A).m[mii][0].r[rii].a[aii].x.k);
							...
							if ((*A).m[mii][0].r[rii].a[aii].N[0]=='C'){rB=1.70;}
	                                                if ((*A).m[mii][0].r[rii].a[aii].N[0]=='O'){rB=1.52;}
							if ((*A).m[mii][0].r[rii].a[aii].N[0]=='N'){rB=1.55;}
							if ((*A).m[mii][0].r[rii].a[aii].N[0]=='S'){rB=1.80;}
							
							//printf("Dist is %f, rA is %f, rB is %f \n\n",d,rA,rB);
							// if the sum of the radii is greater than the distance between them.
							if (rA + rB > d + 0.6){ // 0.6 overlap is deemed acceptable. (Copying chimera:) i
								swtch=1;
	  							soap=2*PI*rA*(rA-d/2-(((rA*rA)-(rB*rB))/(2*d)));
								if (soap>(CSA/4)){
									printf("Residue %d is clashing with residue %d\n",(*A).m[mi][0].r[ri].n,(*A).m[mii][0].r[rii].n);
								}
		...
		printf("resid_soap=%2f for residue %d with residue%d\n",resid_soap,ri,rii);
		if (resid_soap > 2*CSA) {
			printf("\nInternal Clash detected! Residue %d\n",(*A).m[mi][0].r[ri].n);
